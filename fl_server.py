...
class SaveAggregatedFedAvg(FedAvg):
    def aggregate_fit(
        ...
        if aggregated is not None:
            weights, metrics = aggregated
            print(f"[Server] Saving aggregated weights for round {rnd} to aggregated_weights.pkl")
            # weights are numpy arrays (list of ndarrays)
            with open("aggregated_weights.pkl", "wb") as f:
                pickle.dump(weights, f)
...
if __name__ == "__main__":
    ...
    fl.server.start_server(
        server_address="127.0.0.1:8080",
        config=fl.server.ServerConfig(num_rounds=5),  # adjust rounds
        strategy=strategy,
    )

    print("✅ Flower server finished (rounds complete).")
